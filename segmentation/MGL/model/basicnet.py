...
class ConcatNet(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        print("ConcatNet forward x1 shape ", x1.shape, " x2 ", x2.shape, " x3 ", x3.shape, " x4 ", x4.shape)
...
class GraphConvNet(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=False):
        print("GraphConvNet init in_features ", in_features , " out_features ", out_features)
    ...
    def forward(self, x, adj):
        print("GraphConvNet forward x shape ", x.shape, " adj ", adj.shape)
...
class CascadeGCNet(nn.Module):
    def __init__(self, dim, loop):
        ...
        print("CascadeGCNet init dim ", dim, " loop ",loop)
    ...
    def forward(self, x):
        print("GraphNet init x shape ", x.shape)
...
class GraphNet(nn.Module):
    def __init__(self, node_num, dim, normalize_input=False):
        ...
        print("GraphNet init node_num ", node_num, " dim ", dim)
    ...
    def forward(self, x):
        print("GraphNet forward x shape ", x.shape)
        B, C, H, W = x.size()  # 512, 3, 60, 60
...
class ECGraphNet(nn.Module):
    ...
    def forward(self, x, edge):
        print("ECGraphNet forward x shape ", x.shape, " edge ", edge.shape)
        b, c, h, w = x.shape
        device = x.device
...
class MutualNet(nn.Module):
    ...
    def forward(self, edge_x, region_x):
        # project region/edge fature to graph  [3, 512, 60, 60]      [3, 512, 60, 60]
        print("MutualNet forward edge_x ", edge_x.shape, " region_x ", region_x.shape)

        region_graph, region_assign = self.region_proj0(region_x)
        print("MutualNet forward region_graph ", region_graph.shape, " region_assign ", region_assign.shape)
        #        #[3, 512, 32]               [3, 32, 3600]

        edge_graph, edge_assign = self.edge_proj0(edge_x)
        print("MutualNet forward edge_graph ", edge_graph.shape, " edge_assign ", edge_assign.shape)
        #   [3, 512, 32]    [3, 32, 3600]

        edge_graph = self.edge_conv(edge_graph.unsqueeze(3)).squeeze(3)  # => [3, 512, 32]

        # region-edge mutual learning
        region_graph1 = self.region_conv1(region_graph.unsqueeze(3)).squeeze(3)  # => [3, 512, 32]
        region_graph2 = self.region_conv2(region_graph.unsqueeze(3)).squeeze(3)  # => #[3, 512, 32]

        # CGI
        n_edge_x = self.r2e(edge_graph, region_graph1, region_graph2, edge_assign) # => [3, 512, 3600]
        edge_x = edge_x + n_edge_x.view(edge_x.size()).contiguous()  # => [3, 512, 60, 60]

        # edge-region mutual learning
        region_x, edge, region = self.e2r(region_x, region_graph, region_assign, edge_x)
        #[3, 512, 60, 60]  [3, 1, 60, 60]   [3, 1, 60, 60]
