...
#fcn = UNet.UNet(3, 12)
#fcn = FusionNet.Fusionnet(3, 12, 64)
fcn = GCN.GCN(2)
#fcn = SegNet.VGG16_deconv()
#fcn = Deeplab_v3plus.DeepLabv3_plus(nInputChannels=3, n_classes=2, os=16, _print=True)
...
def train(model):
    ...
    for epoch in range(cfg.EPOCH_NUMBER):
        ...
        if max(best) <= train_miou / len(train_data):
            ...
            t.save(net.state_dict(), cfg.RESULT_ROOT + '/weights/FCN_weight/best.pth'.format(epoch))


def evaluate(model):
    net = fcn
    net.load_state_dict(t.load(cfg.RESULT_ROOT + "/weights/FCN_weight/best.pth"))
    net.eval()
...
if __name__ == "__main__":
    ...
    #load trained model
    #pretrained_state_dict = torch.load(cfg.RESULT_ROOT + '/weights/FCN_weight/best.pth')
