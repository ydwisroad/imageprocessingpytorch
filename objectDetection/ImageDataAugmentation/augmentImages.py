...
def copysmallobjects2(image_dir, label_dir, save_base_dir, small_img_dir,
                      times, iCount, area_max=40000, area_min=200):
    image = cv2.imread(image_dir)
    ...
    for small_img_dirs in small_img_dir:
        ...
        # TODO
        # from 3000 to 1500
        roi = suo_fang(image_bbox, area_max=40000, area_min=200)
        print("after resize ", roi.shape)

        new_bboxes = random_add_patches2(cls, roi.shape,
                                         rescale_labels,
                                         image.shape,
                                         paste_number=1,
                                         iou_thresh=0.0001)
