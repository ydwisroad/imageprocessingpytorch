...
def run(imagePath, outputImagePath):
    image = cv2.imread(imagePath)  # a black objects on white image is better
    ...
    index = 0
    for img in imgs:
        cv2.imwrite(outputImagePath + "/" + str(id(img)) +"_"+ str(index) +".jpg", img)
        index = index + 1
        #cv2.imshow("contours", img)
        #cv2.imwrite(outputImagePath + "image.jpg", sure_bg)
        #cv2.waitKey(1943)

if __name__ == '__main__':
    imagePath = "E:/ubuntushare/data/warehousetools01/segmentopencv/scharr/1643535826273.jpg"
    outputImagePath = "E:/ubuntushare/data/warehousetools01/segmentopencv/scharrout/"
    if not os.path.exists(outputImagePath):
        os.mkdir(outputImagePath)
    run(imagePath, outputImagePath)