    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0:
                regression_losses.append(torch.tensor(0).float().to(device)) #.cuda())
                classification_losses.append(torch.tensor(0).float().to(device))  #.cuda())
            ...
            alpha_factor = torch.ones(targets.shape).to(device) * alpha  #.cuda()
            ...
            cls_loss = torch.where(
                torch.ne(targets, -1.0), cls_loss, torch.zeros(cls_loss.shape).to(device))  #.cuda(
            ...
            if positive_indices.sum() > 0:
                ...
                targets = targets/torch.Tensor([[0.1, 0.1, 0.2, 0.2]]).to(device)  #.cuda()
