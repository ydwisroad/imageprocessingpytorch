...
def train(train_loader, model, scheduler, optimizer, epoch, args):
    ...
    for idx, (images, annotations) in enumerate(train_loader):
        images = images.to(device, non_blocking=True).float()  # images.cuda().float()
        annotations = annotations.to(device, non_blocking=True) #.cuda()
...
def test(dataset, model, epoch, args):
    ...
    with torch.no_grad():
        if(args.dataset == 'VOC'):
            mAP, avgPrec = evaluate(dataset, model)
        else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if(args.dataset == 'VOC'):
        ...
        valid_dataset = VOCDetection(root=args.dataset_root, image_sets=[(
            '2012', 'test')], transform=transforms.Compose([Normalizer(), Resizer()]))
    ...
    del checkpoint
    print("distributed args.gpu ", args.gpu)
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    if args.distributed:
        ...
        gpu_ids = [int(i) for i in list(args.gpu_ids)]
        if len(gpu_ids) >= 1:
    ...
    for epoch in range(args.start_epoch, args.num_epoch):
        ...
        if (epoch + 1) % 3 == 0:
            mAP, aPrec = test(valid_dataset, model, epoch, args)
